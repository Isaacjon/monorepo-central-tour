...
type ProfileLanguageChangerProps = {
  lang: string
  dropdownTitle: string
}

const PROFILE_LANGUAGE_OPTIONS_BY_LOCALE: Record<Locale, LanguageOption[]> = {
  ru: [
...
export function ProfileLanguageChanger({
  ...
  const currentLocale = locales.includes(lang as Locale)
    ? (lang as Locale)
    : "ru"
  const options = PROFILE_LANGUAGE_OPTIONS_BY_LOCALE[currentLocale]
