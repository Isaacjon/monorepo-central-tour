...
import type { DateRangePickerProps } from "ui"
import { FlightsFilterDatePicker } from "./flights-filter-date-picker"
import { FlightsFilterLocationFields } from "./flights-filter-location-fields"
...
import { FlightsPassengersPopover } from "../passengers/flights-passengers-popover"
...
export function FlightsFilter({
  ...
  const handlePassengersChange = useCallback(
    (value: FlightPassengersSelection) => setPassengers(value),
    []
  )
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex min-w-0 items-stretch gap-3">
        <FlightsFilterLocationFields
          fromLabel={copy.fromLabel}
          toLabel={copy.toLabel}
          toPlaceholder={copy.toPlaceholder}
          locationSearchPlaceholder={copy.locationSearchPlaceholder}
          noLocationResults={copy.noLocationResults}
          airportsGroupLabel={copy.airportsGroupLabel}
          fromAirportOptions={fromAirportOptions}
          toAirportOptions={toAirportOptions}
          fromSearchValue={fromSearchValue}
          toSearchValue={toSearchValue}
          onFromSearchValueChange={setFromSearchValue}
          onToSearchValueChange={setToSearchValue}
          fromLocation={effectiveFromLocation}
          toLocation={effectiveToLocation}
        ...
        <div className="min-w-0 flex-1">
          <FlightsFilterDatePicker
            selected={travelDateRange}
            onSelect={setTravelDateRange}
            isRoundTrip={tripType === "round-trip"}
            departureLabel={copy.departureLabel}
            returnLabel={copy.returnLabel}
            departurePlaceholder={copy.departurePlaceholder}
            returnPlaceholder={copy.returnPlaceholder}
          />
        </div>
